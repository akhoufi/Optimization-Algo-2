...
def hg(args):
    ...
    try:
        output = check_output(full_command, env=os.environ, universal_newlines=True, shell=True, stderr=subprocess.STDOUT)
...
def run(directory, args, verbose=False):
    ...
    try:
        os.chdir(directory)
        output = check_output_with_print(verbose, args, stderr=subprocess.STDOUT, env=os.environ,
...
def python(directory, args, env=None, verbose=False):
    ...
    try:
        os.chdir(directory)
        output = check_output_with_print(verbose, full_command, stderr=subprocess.STDOUT, env=os.environ,
...
def make(directory, target, verbose=False):
    ...
    try:
        ...
        output = check_output_with_print(verbose, ['make', target], stderr=subprocess.STDOUT, env=os.environ,
                              universal_newlines=True,shell=True)
    except CalledProcessError as e:
        print("ERROR: return value=%i" % e.returncode)
        print(e.output)
